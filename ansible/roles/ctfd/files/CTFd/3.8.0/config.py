# CTFd version 3.8.0 - https://github.com/CTFd/CTFd/blob/3.8.0/CTFd/config.py
import configparser
import json
import os
...
class ServerConfig(object):
    ...
    if DATABASE_URL.startswith("sqlite") is False:
        ...
        SQLALCHEMY_ENGINE_OPTIONS = {
            "pool_size": 128,      # Maximum number of connections in the pool
            "max_overflow": 32,    # Maximum number of connections that can be created after the pool is full
            "pool_timeout": 30,    # Number of seconds to wait before giving up on getting a connection from the pool
            "pool_pre_ping": True  # Check if the connection is alive before returning it from the pool
        }
        # ==== PATCH END ==== #
